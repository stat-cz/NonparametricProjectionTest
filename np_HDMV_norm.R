...
datasim <- normal.sim.data(mu1=mu1, mu2=mu2, Gamma = Gamma,
                           p=p,n1=n1,n2=n2,
...
system.time({np.mean.vector <- normal.sim.all(dataall = datasim,cores = 5,
                                              trace = TRUE)})


(filename=paste("np_norm_null_n1",n1,"_n2",n2,"_p",p,"_rho2",rho,"_B",B,"_Nsim",Nsim,"_dense.RData",sep =""))
save(np.mean.vector=np.mean.vector,file=filename)
