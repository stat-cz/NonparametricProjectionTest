...
## MA model
rho <- rho2 <- 0
(pattern=paste("np_norm_null_n1",n1,"_n2",n2,"_p",p,"_rho2",rho2,"_B",B,"_Nsim",Nsim,".RData",sep =""))

## dense 
rho <- rho2 <- 0.5
(pattern=paste("np_norm_null_n1",n1,"_n2",n2,"_p",p,"_rho2",rho2,"_B",B,"_Nsim",Nsim,"_dense.RData",sep =""))
...
collect_statistics <- function(dir,pattern,n1,n2,pi,rho,Nsim=100000) 
{
  ...
  PT1 <- np.mean.vector$PT1
  PT2 <- np.mean.vector$PT2
  PT3 <- np.mean.vector$PT3
  npbs <- np.mean.vector$npBS
  npcq <- np.mean.vector$npCQ
  nppt1 <- np.mean.vector$npPT1
  nppt2 <- np.mean.vector$npPT2
  nppt3 <- np.mean.vector$npPT3
...
ggsave(file=paste("pvalue_point_non_mvtest_n1",n1,"_n2",n2,".png",sep=""),plot=pvalue.plot,width=10,height=10)
ggsave(file=paste("pvalue_point_non_mvtest_n1",n1,"_n2",n2,".pdf",sep=""),plot=pvalue.plot,width=10,height=10)
pvalue.plot
